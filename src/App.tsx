...
import {
  defaultSalesStatusSettings,
  deriveDealFromActivity,
  detailStagesBySalesStage,
  getSalesOverview,
  getSalesPeriodSummary,
  pipelineStages,
  type RiskDeal,
  type SalesDealWithStatus,
  type SalesPeriod,
} from "./services/salesOverviewService";
import {
  calculateExistingCustomerStatus,
  getExistingCustomerStatusReason,
  getExistingCustomerSummary,
  opportunityToSalesDeal,
} from "./services/existingCustomerService";
import { formatCurrency, formatNumber, today } from "./services/format";

type ViewKey = "documents" | "salesStatus" | "existingCustomers" | "recommendedActions" | "pricing";
type SalesFilter = "all" | "신규 인입" | "신규 제안" | "active" | "지연" | "위험" | "보류" | "수주" | "실패";
type HeaderCopy = {
  title: string;
  description: string;
};
...
type NewSalesForm = {
  salesType: "신규 인입" | "신규 제안";
  customerName: string;
  industry: string;
  contactName: string;
  contactDepartment: string;
  contactPosition: string;
  contactRole: CustomerContact["role"];
  contactPhone: string;
  contactEmail: string;
  customerNote: string;
  salesNote: string;
  ...
  competitorInvolved: "no" | "yes";
  requestedDiscountRate: string;
...
type CustomerContactForm = {
  ...
  phone: string;
  email: string;
  preferredContactMethod: NonNullable<CustomerContact["preferredContactMethod"]>;
  influenceLevel: NonNullable<CustomerContact["influenceLevel"]>;
  isPrimary: boolean;
  note: string;
};

type DocumentDealForm = {
  customerName: string;
  title: string;
  industry: string;
  expectedRevenue: string;
...
type CustomerOpportunityForm = {
  opportunityType: CustomerOpportunity["opportunityType"];
  title: string;
  expectedRevenue: string;
  expectedDate: string;
  owner: string;
...
type SavedDocument = DocumentDraft & {
  customerId: string;
  dealId: string;
  savedStatus: "초안 저장" | "고객 발송 완료" | "내부 검토 중";
  lastDownloadedAt?: string;
  supportedFormats: ExportFormat[];
};

type SavedPricingGuide = {
  id: string;
  customerId: string;
  dealId: string;
  customerName: string;
...
type SalesTableViewMode = "table" | "card";
...
type SalesColumnKey =
  ...
  | "source"
  | "memo";
type SalesSortState = { key: SalesColumnKey; direction: SalesSortDirection } | null;
type CardSortOption = "default" | "risk" | "due" | "revenue" | "lastContactOld" | "customer";
...
const defaultSalesColumnKeys: SalesColumnKey[] = [
  "customer",
  "deal",
  "status",
  "salesStage",
...
const optionalSalesColumnKeys: SalesColumnKey[] = [
  "detailStage",
  "phone",
  "notes",
  "owner",
...
const salesColumnMeta: Record<SalesColumnKey, { label: string; width: number; required?: boolean; sortable?: boolean }> = {
...
const salesStatusSortOrder: SalesStatus[] = ["위험", "지연", "주의", "보류", "정상", "수주", "실패"];
...
const documentTones: DocumentTone[] = ["공식적", "정중함", "간결함", "영업 친화적", "기술 설명 중심", "내부 보고용"];

const documentTypeCards: Record<DocumentType, { description: string; examples: string; recommended: string }> = {
  메일: {
...
const contactRoles: CustomerContact["role"][] = ["실무 담당자", "기술 담당자", "구매 담당자", "의사결정자", "최종 승인권자", "파트너 담당자", "기타"];
const contactMethods: NonNullable<CustomerContact["preferredContactMethod"]>[] = ["전화", "이메일", "문자", "메신저", "방문 미팅", "미확인"];
const influenceLevels: NonNullable<CustomerContact["influenceLevel"]>[] = ["낮음", "보통", "높음", "핵심"];
...
const defaultInquiry: Record<string, string> = {
...
const priorityLabels: Record<SalesTask["priority"], string> = {
  high: "높음",
  medium: "중간",
  low: "낮음",
};

const headerCopy: Record<ViewKey, HeaderCopy> = {
  documents: {
    title: "문서 만들기",
...
function toDocumentCustomer(deal?: SalesDealWithStatus): Customer {
  if (!deal) return crmData.customers[0];
  return {
    id: deal.customerId,
...
function toDocumentDeal(deal?: SalesDealWithStatus): Deal {
...
function toDocumentLicense(deal?: SalesDealWithStatus): License {
...
function draftToText(draft: DocumentDraft) {
  return [
    draft.title,
...
function getPrimaryContact(contacts: CustomerContact[], customerId: string, preferredId?: string) {
...
function contactSummary(contact: CustomerContact | null) {
  if (!contact) return "미등록";
  return `${contact.name}${contact.department ? ` / ${contact.department}` : ""}`;
}

function contactDetail(contact: CustomerContact | null) {
  if (!contact) return "미등록";
  return [contact.name, contact.department, contact.position, contact.role].filter(Boolean).join(" / ");
}

function getCustomerNote(notes: CustomerNotes[], customerId: string) {
  return notes.find((note) => note.customerId === customerId) ?? { customerId };
}

function hasCustomerNotes(note: CustomerNotes | undefined) {
...
function shortNoteLabels(note: CustomerNotes | undefined) {
...
function customerNoteTooltip(note: CustomerNotes | undefined) {
  if (!note || !hasCustomerNotes(note)) return "등록된 특이사항이 없습니다.";
  return [
...
function loadSalesTableColumns() {
  try {
    ...
    if (!saved) return defaultSalesColumnKeys;
    const parsed = JSON.parse(saved) as SalesColumnKey[];
    const allColumns = new Set<SalesColumnKey>([...defaultSalesColumnKeys, ...optionalSalesColumnKeys]);
...
function loadSalesTableViewMode(): SalesTableViewMode {
  try {
...
function loadSalesSortState(): SalesSortState {
  try {
    const saved = localStorage.getItem(salesTableSortStorageKey);
    if (!saved) return null;
    const parsed = JSON.parse(saved) as SalesSortState;
    if (!parsed) return null;
    if (!salesColumnMeta[parsed.key]?.sortable) return null;
...
function defaultDealSort(a: SalesDealWithStatus, b: SalesDealWithStatus) {
...
function getDealSortValue(deal: SalesDealWithStatus, key: SalesColumnKey, contacts: CustomerContact[]) {
  const primaryContact = getPrimaryContact(contacts, deal.customerId, deal.primaryContactId);
  if (key === "customer") return deal.customerName;
  if (key === "deal") return deal.title;
  if (key === "salesStage") return pipelineStages.indexOf(deal.salesStage);
  if (key === "detailStage") return deal.detailStage;
  if (key === "status") return salesStatusSortOrder.indexOf(deal.computedStatus);
  if (key === "expectedRevenue") return deal.expectedRevenue;
  if (key === "owner") return deal.owner;
  if (key === "contact") return contactSummary(primaryContact);
  if (key === "lastContact") return deal.lastContactDate;
  if (key === "dueDate") return deal.nextActionDueDate || "9999-12-31";
  if (key === "requestedDiscount") return deal.requestedDiscountRate ?? -1;
  if (key === "industry") return deal.industry;
  return "";
}

function sortSalesDeals(deals: SalesDealWithStatus[], sortState: SalesSortState, contacts: CustomerContact[]) {
  const sorted = [...deals];
  if (!sortState) return sorted.sort(defaultDealSort);
  sorted.sort((a, b) => {
...
function cardSortOptionFromState(sortState: SalesSortState): CardSortOption {
  if (!sortState) return "default";
  if (sortState.key === "status" && sortState.direction === "asc") return "risk";
  if (sortState.key === "dueDate" && sortState.direction === "asc") return "due";
  if (sortState.key === "expectedRevenue" && sortState.direction === "desc") return "revenue";
  if (sortState.key === "lastContact" && sortState.direction === "asc") return "lastContactOld";
  if (sortState.key === "customer" && sortState.direction === "asc") return "customer";
  return "default";
}

function sortStateFromCardOption(option: CardSortOption): SalesSortState {
  if (option === "risk") return { key: "status", direction: "asc" };
  if (option === "due") return { key: "dueDate", direction: "asc" };
  if (option === "revenue") return { key: "expectedRevenue", direction: "desc" };
  if (option === "lastContactOld") return { key: "lastContact", direction: "asc" };
  if (option === "customer") return { key: "customer", direction: "asc" };
  return null;
}

function loadPricingSettings(): PricingGuideSettings {
  try {
    const saved = localStorage.getItem(pricingSettingsStorageKey);
    if (!saved) return defaultPricingGuideSettings;
    const parsed = JSON.parse(saved) as Partial<PricingGuideSettings>;
    return {
      ...defaultPricingGuideSettings,
      ...parsed,
...
const emptySalesForm: NewSalesForm = {
  salesType: "신규 인입",
  customerName: "",
  industry: "",
  contactName: "",
...
function App() {
  const [authUser, setAuthUser] = useState<typeof currentUser | null>(() => {
...
function CrmShell({ user, onLogout }: { user: typeof currentUser; onLogout: () => void }) {
  const [activeView, setActiveView] = useState<ViewKey>("salesStatus");
  ...
  const [tone, setTone] = useState<DocumentTone>("공식적");
  const [inquiry, setInquiry] = useState(defaultInquiry[customerId]);
  const [draft, setDraft] = useState<DocumentDraft | null>(null);
  const [salesDeals, setSalesDeals] = useState<SalesDeal[]>(crmData.salesDeals);
  const [salesTasks, setSalesTasks] = useState<SalesTask[]>(crmData.salesTasks);
  const [salesActivities, setSalesActivities] = useState<SalesActivity[]>(crmData.salesActivities);
  const [existingCustomers, setExistingCustomers] = useState<ExistingCustomer[]>(crmData.existingCustomers);
  const [customerOpportunities, setCustomerOpportunities] = useState<CustomerOpportunity[]>(crmData.customerOpportunities);
  const [customerContacts, setCustomerContacts] = useState<CustomerContact[]>(crmData.customerContacts);
  const [customerNotes, setCustomerNotes] = useState<CustomerNotes[]>(crmData.customerNotes);
  const [savedDocuments, setSavedDocuments] = useState<SavedDocument[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("sales-crm-document-history") || "[]") as SavedDocument[];
    } catch {
      return [];
    }
  });
  const [savedPricingGuides, setSavedPricingGuides] = useState<SavedPricingGuide[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("sales-crm-pricing-history") || "[]") as SavedPricingGuide[];
    } catch {
      return [];
    }
  });
  const [statusSettings, setStatusSettings] = useState<SalesStatusRuleSettings>(defaultSalesStatusSettings);
  const [pricingSettings, setPricingSettings] = useState<PricingGuideSettings>(loadPricingSettings);
  ...
  const [pricingInput, setPricingInput] = useState<PricingInput>(() =>
    createDefaultPricingInput(crmData.deals[0], crmData.customers[0].industry, crmData.customers[0].installedModules),
  );
  ...
  const opportunitySalesDeals = useMemo(
    () => customerOpportunities
      ...
      .filter((item): item is SalesDeal => Boolean(item)),
    [customerOpportunities, existingCustomers],
  );
  ...
  const salesOverview = useMemo(
    () => getSalesOverview(allSalesDeals, salesTasks, salesActivities, statusSettings),
    [allSalesDeals, salesTasks, salesActivities, statusSettings],
  );
  ...
  const pricingGuide = useMemo(
    () => calculatePricingGuide(deal, industryCases, pricingInput, pricingSettings),
    [deal, industryCases, pricingInput, pricingSettings],
  );
  const currentHeader = headerCopy[activeView];

  function handleCustomerChange(nextCustomerId: string) {
    const nextDeal = crmData.deals.find((item) => item.customerId === nextCustomerId) as Deal;
  ...
  function savePricingSettings(settings: PricingGuideSettings) {
    localStorage.setItem(pricingSettingsStorageKey, JSON.stringify(settings));
    setPricingSettings(settings);
  }

  function saveDocument(document: SavedDocument) {
  ...
  function updateDocumentDownload(documentId: string) {
    setSavedDocuments((current) => {
  ...
  function savePricingGuideResult(record: SavedPricingGuide) {
  ...
  function createDraft(input: DocumentDraftInput) {
    setDraft(
      generateDocumentDraft(input, buildDocumentContext(
        customer,
        deal,
        documentLicense,
        documentSalesDeal,
        documentActivities,
        documentPricingGuide,
        documentIndustryCases,
      )),
    );
  }

  function addDocumentDeal(form: DocumentDealForm) {
  ...
  function addSalesDeal(form: NewSalesForm) {
    const id = `sd-${Date.now()}`;
    const matchedCustomer = crmData.customers.find((item) =>
      [item.name, item.displayName, item.baseName].filter(Boolean).includes(form.customerName.trim()),
    );
  ...
  function addActivity(deal: SalesDealWithStatus, form: ActivityForm) {
    const activity: SalesActivity = {
      id: `act-${Date.now()}`,
      dealId: deal.id,
      customerId: deal.customerId,
    ...
    if (["수주", "계약 완료", "발주 완료"].includes(activity.activityType)) {
      setExistingCustomers((current) => {
        if (current.some((item) => item.customerId === deal.customerId && item.customerName === deal.customerName)) return current;
  ...
  function updateActivity(deal: SalesDealWithStatus, form: ActivityForm) {
    if (!form.id) return;
  ...
  function deleteActivity(deal: SalesDealWithStatus, activityId: string) {
    setSalesActivities((current) =>
      current.map((activity) => (activity.id === activityId ? { ...activity, deletedAt: formatDate(today), updatedAt: formatDate(today) } : activity)),
    );
    setSalesDeals((current) => current.map((item) => (item.id === deal.id ? { ...item, lastUpdatedAt: formatDate(today) } : item)));
  }

  function updateDealNextAction(deal: SalesDealWithStatus, nextAction: string, nextActionDueDate: string) {
    setSalesDeals((current) =>
      current.map((item) =>
        item.id === deal.id
          ? {
              ...item,
  ...
  function saveCustomerContact(customerId: string, form: CustomerContactForm) {
  ...
  function setPrimaryCustomerContact(contactId: string, customerId: string) {
  ...
  function saveCustomerNotes(customerId: string, note: CustomerNotes) {
    setCustomerNotes((current) => [note, ...current.filter((item) => item.customerId !== customerId)]);
  }

  function addCustomerOpportunity(existingCustomer: ExistingCustomer, form: CustomerOpportunityForm) {
    ...
    if (form.activeInSalesPipeline) setActiveView("salesStatus");
  }
  ...
  return (
    <div className="appShell">
      ...
      <main className="workspace">
          ...
          <section className="mainPanel">
            {activeView === "documents" && (
              <DocumentsView
                ...
                deal={documentDeal}
                salesDeals={salesOverview.dealsWithStatus}
                selectedDealId={documentSalesDeal?.id ?? ""}
                documentType={documentType}
                tone={tone}
                inquiry={inquiry}
                activities={documentActivities}
                contacts={customerContacts.filter((contact) => contact.customerId === documentCustomer.id)}
                pricingGuide={documentPricingGuide}
                evidenceCases={documentIndustryCases.slice(0, 3)}
                draft={draft}
                onDocumentDealChange={(dealId) => {
                  setDocumentDealId(dealId);
                  setDraft(null);
                }}
                onAddDocumentDeal={addDocumentDeal}
                onDocumentTypeChange={setDocumentType}
                onToneChange={setTone}
                onInquiryChange={setInquiry}
                onGenerate={createDraft}
                onSaveDocument={saveDocument}
                onDownloadDocument={updateDocumentDownload}
                onSaveAsActivity={(activity) => setSalesActivities((current) => [activity, ...current])}
              />
            )}
            {activeView === "salesStatus" && (
              <SalesOverviewView
                overview={salesOverview}
                activities={salesActivities}
                contacts={customerContacts}
                notes={customerNotes}
                salesTarget={crmData.salesTarget}
                settings={statusSettings}
                onSettingsChange={setStatusSettings}
                onAddDeal={addSalesDeal}
                onAddActivity={addActivity}
                onNavigate={setActiveView}
                onOpenDeal={(dealId) => setDrawerDealId(dealId)}
              />
            )}
            {activeView === "existingCustomers" && (
              <ExistingCustomersView
                ...
                contacts={customerContacts}
                notes={customerNotes}
                onSaveContact={saveCustomerContact}
                onDeleteContact={deleteCustomerContact}
                onSetPrimaryContact={setPrimaryCustomerContact}
                onSaveNotes={saveCustomerNotes}
                onCreateOpportunity={addCustomerOpportunity}
                onNavigate={setActiveView}
              />
            )}
            {activeView === "recommendedActions" && (
              <RecommendedActionsView
                tasks={salesOverview.upcomingTasks}
                riskDeals={salesOverview.riskDeals}
                deals={salesOverview.dealsWithStatus}
                selectedCustomerName={customerDisplayName(customer)}
                selectedCustomerDealCount={salesOverview.dealsWithStatus.filter((item) => item.customerId === customer.id).length}
                selectedCustomerTaskCount={salesTasks.filter((task) =>
                  salesOverview.dealsWithStatus.some((selectedDeal) => selectedDeal.customerId === customer.id && selectedDeal.id === task.dealId),
                ).length}
                onNavigate={setActiveView}
                onOpenDeal={(dealId) => setDrawerDealId(dealId)}
              />
            ...
            {activeView === "pricing" && (
              <PricingView
                ...
                onChange={(nextInput) => setPricingInput((current) => ({ ...current, ...nextInput }))}
                onSettingsChange={savePricingSettings}
        ...
        {drawerDeal && (
          <DetailDrawerShell label={`${drawerDeal.customerName} 영업 상세`} onClose={() => setDrawerDealId(null)}>
            <DealDetailPanel
              deal={drawerDeal}
              activities={salesActivities.filter((activity) => activity.dealId === drawerDeal.id)}
              documents={savedDocuments.filter((document) => document.dealId === drawerDeal.id || document.customerId === drawerDeal.customerId)}
              pricingGuides={savedPricingGuides.filter((item) => item.dealId === drawerDeal.id || item.customerId === drawerDeal.customerId)}
              contacts={customerContacts.filter((contact) => contact.customerId === drawerDeal.customerId)}
              customerNote={getCustomerNote(customerNotes, drawerDeal.customerId)}
              onClose={() => setDrawerDealId(null)}
              onAddActivity={addActivity}
              onUpdateActivity={updateActivity}
              onDeleteActivity={deleteActivity}
              onSaveContact={(form) => saveCustomerContact(drawerDeal.customerId, form)}
              onDeleteContact={(contactId) => deleteCustomerContact(contactId, drawerDeal.customerId)}
              onSetPrimaryContact={(contactId) => setPrimaryCustomerContact(contactId, drawerDeal.customerId)}
              onSaveNotes={(note) => saveCustomerNotes(drawerDeal.customerId, note)}
              onUpdateNextAction={updateDealNextAction}
              onNavigate={setActiveView}
            />
          </DetailDrawerShell>
        )}
...
function LoginView({ onLogin }: { onLogin: (email: string, password: string) => boolean }) {
...
function RecommendedActionsView({
  tasks,
  riskDeals,
  deals,
  selectedCustomerName,
  selectedCustomerDealCount,
  selectedCustomerTaskCount,
  onNavigate,
  onOpenDeal,
}: {
  tasks: SalesTask[];
  riskDeals: RiskDeal[];
  deals: SalesDealWithStatus[];
  selectedCustomerName: string;
  selectedCustomerDealCount: number;
  selectedCustomerTaskCount: number;
  ...
  const priorityItems = [
    ...
    ...riskDeals.slice(0, 3).map((deal) => ({
      ...
      target: deal.requestedDiscountRate && deal.requestedDiscountRate >= 15 ? ("pricing" as ViewKey) : ("documents" as ViewKey),
      dealId: deal.id,
    })),
    ...weekTasks.map((task) => ({
      id: `week-${task.id}`,
      ...
      <section className="actionRail" aria-label="추천 액션">
        ...
        <div className="actionCardStack">
          {priorityItems.map((item) => (
            <article className="actionCard clickableCard" key={item.id} onClick={() => onOpenDeal(item.dealId)}>
...
function NextActionPanel({
  ...
  return (
    <aside className="nextActionPanel" aria-label="다음 액션">
      <div className="railHeader">
        <ClipboardList size={18} />
        <strong>다음 액션</strong>
      </div>
      ...
      <div className="actionCardStack">
        ...
        {priorityItems.length > visiblePriorityItems.length && (
          <button className="secondaryButton fullButton" type="button" onClick={() => onNavigate("recommendedActions")}>
...
function NavButton({
  ...
  return (
    <button className={`navItem ${activeView === view ? "active" : ""}`} onClick={() => onClick(view)}>
      {icon}
      {label}
    </button>
  );
...
function ExistingCustomersView({
  customers,
  opportunities,
  activities,
  documents,
  ...
  notes: CustomerNotes[];
  onSaveContact: (customerId: string, form: CustomerContactForm) => void;
  onDeleteContact: (contactId: string, customerId: string) => void;
  onSetPrimaryContact: (contactId: string, customerId: string) => void;
  onSaveNotes: (customerId: string, note: CustomerNotes) => void;
  onCreateOpportunity: (customer: ExistingCustomer, form: CustomerOpportunityForm) => void;
  onNavigate: (view: ViewKey) => void;
}) {
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | ExistingCustomer["status"]>("all");
  ...
  return (
    <div className="featureStack">
      ...
      <section className="panel">
        ...
        <div className="existingCustomerTable">
          ...
          {filteredCustomers.map((customer) => {
            ...
            return (
              <button className={`existingCustomerRow ${selectedCustomerId === customer.id ? "selectedRow" : ""}`} key={customer.id} onClick={() => setSelectedCustomerId(customer.id)}>
      ...
      {selectedCustomer && (
        <DetailDrawerShell label={`${selectedCustomer.customerName} 기존 고객 상세`} onClose={() => setSelectedCustomerId(null)}>
          <ExistingCustomerDetailPanel
            customer={selectedCustomer}
            opportunities={opportunities.filter((opportunity) => opportunity.existingCustomerId === selectedCustomer.id)}
            activities={activities.filter((activity) => activity.customerId === selectedCustomer.customerId)}
            documents={documents.filter((document) => document.customerId === selectedCustomer.customerId)}
            contacts={contacts.filter((contact) => contact.customerId === selectedCustomer.customerId)}
            customerNote={getCustomerNote(notes, selectedCustomer.customerId)}
            onSaveContact={(form) => onSaveContact(selectedCustomer.customerId, form)}
            onDeleteContact={(contactId) => onDeleteContact(contactId, selectedCustomer.customerId)}
            onSetPrimaryContact={(contactId) => onSetPrimaryContact(contactId, selectedCustomer.customerId)}
            onSaveNotes={(note) => onSaveNotes(selectedCustomer.customerId, note)}
...
function ExistingCustomerDetailPanel({
  customer,
  opportunities,
  activities,
  documents,
  ...
  customer: ExistingCustomer & { computedStatus?: ExistingCustomer["status"]; computedStatusReason?: string };
  opportunities: CustomerOpportunity[];
  activities: SalesActivity[];
  documents: SavedDocument[];
  contacts: CustomerContact[];
  customerNote: CustomerNotes;
  onSaveContact: (form: CustomerContactForm) => void;
  onDeleteContact: (contactId: string) => void;
  onSetPrimaryContact: (contactId: string) => void;
  onSaveNotes: (note: CustomerNotes) => void;
  onCreateOpportunity: (customer: ExistingCustomer, form: CustomerOpportunityForm) => void;
  onNavigate: (view: ViewKey) => void;
  onClose: () => void;
}) {
  const [activeTab, setActiveTab] = useState<"overview" | "contacts" | "license" | "history" | "opportunities" | "documents" | "memo">("overview");
...
function ContactManager({
  ...
  contacts: CustomerContact[];
  customerId: string;
  onSave: (form: CustomerContactForm) => void;
  onDelete: (contactId: string) => void;
  onSetPrimary: (contactId: string) => void;
}) {
  const emptyContactForm: CustomerContactForm = {
    name: "",
    department: "",
    position: "",
    role: "실무 담당자",
  ...
  const [form, setForm] = useState<CustomerContactForm>(emptyContactForm);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [feedback, setFeedback] = useState("");
  function update<K extends keyof CustomerContactForm>(key: K, value: CustomerContactForm[K]) {
  ...
  function edit(contact: CustomerContact) {
  ...
  return (
    <section className="drawerSection contactManager">
      ...
      <div className="contactList">
        {contacts.length ? contacts.map((contact) => (
          <article className={`contactCard ${contact.isPrimary ? "primaryContact" : ""}`} key={contact.id}>
            <div>
            ...
            <div className="contactActions">
              ...
              <button className="secondaryButton compactButton dangerButton" onClick={() => { if (window.confirm("고객 담당자를 삭제하시겠습니까?")) { onDelete(contact.id); setFeedback("고객 담당자가 삭제되었습니다."); } }}>삭제</button>
            </div>
          </article>
        )) : <EmptyState text="등록된 고객 담당자가 없습니다. 담당자 추가로 첫 연락처를 기록하세요." />}
      </div>
      <div className="activityForm">
        ...
        <div className="formGrid denseForm">
          ...
          <FieldLabel label="역할"><select value={form.role} onChange={(event) => update("role", event.target.value as CustomerContact["role"])}>{contactRoles.map((role) => <option key={role}>{role}</option>)}</select></FieldLabel>
          ...
          <FieldLabel label="선호 연락 방식"><select value={form.preferredContactMethod} onChange={(event) => update("preferredContactMethod", event.target.value as CustomerContactForm["preferredContactMethod"])}>{contactMethods.map((method) => <option key={method}>{method}</option>)}</select></FieldLabel>
          <FieldLabel label="의사결정 영향도"><select value={form.influenceLevel} onChange={(event) => update("influenceLevel", event.target.value as CustomerContactForm["influenceLevel"])}>{influenceLevels.map((level) => <option key={level}>{level}</option>)}</select></FieldLabel>
        </div>
...
function CustomerNotesPanel({ note, editable = false, onSave }: { note: CustomerNotes; editable?: boolean; onSave?: (note: CustomerNotes) => void }) {
  const [draft, setDraft] = useState<CustomerNotes>(note);
  const [feedback, setFeedback] = useState("");
  useEffect(() => {
    setDraft(note);
    setFeedback("");
  ...
  function update<K extends keyof CustomerNotes>(key: K, value: CustomerNotes[K]) {
    setDraft((current) => ({ ...current, [key]: value }));
  }
  const rows: Array<[keyof CustomerNotes, string]> = [
  ...
  return (
    <section className="drawerSection notesPanel">
      <div className="panelHeader subsectionHeader">
...
function CustomerOpportunityFormPanel({ customer, onSubmit }: { customer: ExistingCustomer; onSubmit: (form: CustomerOpportunityForm) => void }) {
  const [form, setForm] = useState<CustomerOpportunityForm>({
  ...
  const required: Array<[keyof CustomerOpportunityForm, string]> = [["opportunityType", "기회 유형"], ["title", "기회명"], ["expectedRevenue", "예상 매출"], ["expectedDate", "예상 시점"], ["owner", "담당자"], ["nextAction", "다음 액션"], ["nextActionDueDate", "다음 액션 기한"]];
  function update<K extends keyof CustomerOpportunityForm>(key: K, value: CustomerOpportunityForm[K]) {
    setForm((current) => ({ ...current, [key]: value }));
  }
  function submit() {
  ...
  return (
    <div className="activityForm">
      ...
      <div className="formGrid denseForm">
        <FieldLabel label="기회 유형" required><select value={form.opportunityType} onChange={(event) => update("opportunityType", event.target.value as CustomerOpportunity["opportunityType"])}>{["재결제", "라이선스 연장", "라이선스 증설", "유지보수 갱신", "고도화 영업", "추가 모듈 제안", "기타"].map((item) => <option key={item}>{item}</option>)}</select></FieldLabel>
...
function DocumentsView({
  ...
  customer: Customer;
  deal: Deal;
  salesDeals: SalesDealWithStatus[];
  ...
  activities: SalesActivity[];
  contacts: CustomerContact[];
  pricingGuide: ReturnType<typeof calculatePricingGuide>;
  evidenceCases: WinLossCase[];
  draft: DocumentDraft | null;
  onDocumentDealChange: (dealId: string) => void;
  onAddDocumentDeal: (form: DocumentDealForm) => void;
  onDocumentTypeChange: (value: DocumentType) => void;
  onToneChange: (value: DocumentTone) => void;
  onInquiryChange: (value: string) => void;
  onGenerate: (input: DocumentDraftInput) => void;
  onSaveDocument: (document: SavedDocument) => void;
  onDownloadDocument: (documentId: string) => void;
  onSaveAsActivity: (activity: SalesActivity) => void;
}) {
  const [feedback, setFeedback] = useState<{ type: "success" | "error"; message: string } | null>(null);
  const [showAddDocumentDeal, setShowAddDocumentDeal] = useState(false);
  const [documentStep, setDocumentStep] = useState<1 | 2 | 3 | 4>(1);
  const [newDocumentDeal, setNewDocumentDeal] = useState<DocumentDealForm>({
    customerName: "",
    title: "",
    industry: "",
    expectedRevenue: "",
  ...
  const [audience, setAudience] = useState<"고객 전달용" | "내부용">("고객 전달용");
  ...
  const [sendOption, setSendOption] = useState<"초안만 저장" | "고객 발송 완료로 저장" | "내부 검토 중으로 저장">("초안만 저장");
  const [exportFormat, setExportFormat] = useState<ExportFormat>(recommendedExportFormat(documentType));
  const [documentLength, setDocumentLength] = useState<"짧게" | "보통" | "자세히">("보통");
  const [outputDetail, setOutputDetail] = useState<"요약형" | "표준형" | "상세형">("표준형");
  const [styleStrength, setStyleStrength] = useState<"매우 공식적" | "일반 공식" | "실무형" | "친근한 영업형">("일반 공식");
  const [includeItems, setIncludeItems] = useState<string[]>(["고객 요구사항 요약", "최근 영업 이력", "제품/모듈 구성", "다음 액션", "첨부 안내"]);
  const [excludeItems, setExcludeItems] = useState<string[]>(["내부 메모 제외", "마진 정보 제외", "가격 방어 논리 제외", "담당자 개인 메모 제외"]);
  ...
  useEffect(() => {
    if (!primaryDocumentContact) return;
    setForm((current) => ({
      ...current,
  ...
  function documentInput(): DocumentDraftInput {
    return {
      ...form,
      documentType,
      templateName,
  ...
  function handleSaveAsActivity() {
    ...
    const activityType: SalesActivity["activityType"] =
      documentType === "견적서" ? "견적서 발송" :
    ...
    <div className="featureStack">
      ...
      <div className="stepper" aria-label="문서 만들기 단계">
        {[
          ...
          <button
            key={step}
            className={documentStep === step ? "active" : ""}
            type="button"
            onClick={() => setDocumentStep(step as 1 | 2 | 3 | 4)}
          >
            <strong>{title}</strong>
            <span>{desc}</span>
          </button>
      ...
      {documentStep === 1 && (
        <section className="documentTypeGrid" aria-label="문서 유형 선택 카드">
          {documentTypes.map((type) => (
            <button
              className={`documentTypeCard ${documentType === type ? "active" : ""}`}
              key={type}
              type="button"
              onClick={() => {
      ...
      <div className="splitGrid">
        <section className="panel">
          ...
          <div className="formGrid">
            <label>
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  if (firstDeal) onDocumentDealChange(firstDeal.id);
                }}
              >
            ...
            <label>
              제출 톤
              <select value={tone} onChange={(event) => onToneChange(event.target.value as DocumentTone)}>
            ...
            <label>
              문서 구분
              <select value={audience} onChange={(event) => setAudience(event.target.value as "고객 전달용" | "내부용")}>
          ...
          {documentType === "견적서" && (
            <div className="documentTypeHint">
              최종 제안 금액은 적용 할인율과 예상 매출을 기준으로 자동 계산되어 견적서 표와 Excel 다운로드에 반영됩니다.
            </div>
          )}
          {documentStep === 1 && showAddDocumentDeal && (
            <section className="inlinePanel">
              <div className="panelTitle">
                <Plus size={18} />
          ...
          <section className={documentStep === 3 ? "inlinePanel documentHistorySelector" : "stepHidden"}>
            ...
            {recentActivities.length ? recentActivities.map((activity) => (
              <label className="checkboxRow" key={activity.id}>
                <input
                  type="checkbox"
                  checked={selectedActivityIds.includes(activity.id)}
                  onChange={(event) => setSelectedActivityIds((current) =>
                    event.target.checked ? [...current, activity.id] : current.filter((id) => id !== activity.id),
                  )}
                />
                <span>{activity.activityDate} {activity.activityType}: {activity.title}</span>
              </label>
            )) : <EmptyState text="선택 가능한 영업 이력이 없습니다." />}
          </section>
          ...
          <section className={documentStep === 3 ? "inlinePanel documentHistorySelector" : "stepHidden"}>
            ...
            <div className="formGrid denseForm">
              <label>문서 분량<select value={documentLength} onChange={(event) => setDocumentLength(event.target.value as typeof documentLength)}>{["짧게", "보통", "자세히"].map((item) => <option key={item}>{item}</option>)}</select></label>
              <label>출력 상세도<select value={outputDetail} onChange={(event) => setOutputDetail(event.target.value as typeof outputDetail)}>{["요약형", "표준형", "상세형"].map((item) => <option key={item}>{item}</option>)}</select></label>
              <label>문체 강도<select value={styleStrength} onChange={(event) => setStyleStrength(event.target.value as typeof styleStrength)}>{["매우 공식적", "일반 공식", "실무형", "친근한 영업형"].map((item) => <option key={item}>{item}</option>)}</select></label>
            </div>
            <div className="optionChecklist">
              <strong>포함할 내용</strong>
              {documentIncludeOptions.map((item) => (
            ...
            <div className="optionChecklist">
              ...
              {documentExcludeOptions.map((item) => (
                <label className="checkboxRow" key={item}>
          ...
          {documentStep === 3 && includePricingGuide && (
            <div className="documentPricingBox">
          ...
          <div className="buttonRow stepNav">
            {documentStep > 1 && <button className="secondaryButton" type="button" onClick={() => setDocumentStep((documentStep - 1) as 1 | 2 | 3 | 4)}>이전</button>}
            {documentStep < 4 && <button className="secondaryButton" type="button" onClick={() => setDocumentStep((documentStep + 1) as 1 | 2 | 3 | 4)}>다음</button>}
          ...
          {draft ? (
            <>
              ...
              <div className="buttonRow">
                ...
                <select value={sendOption} onChange={(event) => setSendOption(event.target.value as typeof sendOption)}>
              ...
              <div className="downloadBox">
                ...
                <select value={exportFormat} onChange={(event) => setExportFormat(event.target.value as ExportFormat)}>
                  {(["PDF", "Word", "Excel"] as ExportFormat[]).map((format) => (
                    <option key={format} value={format} disabled={!getSupportedExportFormats(draft.documentType).includes(format)}>
...
function DocumentTypeFields({
  ...
  onChange: (value: Partial<Record<string, string | number | boolean>>) => void;
  deal: Deal;
  pricingGuide: ReturnType<typeof calculatePricingGuide>;
}) {
  if (documentType === "메일") {
...
function SalesOverviewView({
  ...
  overview: ReturnType<typeof getSalesOverview>;
  activities: SalesActivity[];
  contacts: CustomerContact[];
  notes: CustomerNotes[];
  salesTarget: typeof crmData.salesTarget;
  settings: SalesStatusRuleSettings;
  onSettingsChange: (settings: SalesStatusRuleSettings) => void;
  onAddDeal: (form: NewSalesForm) => void;
  onAddActivity: (deal: SalesDealWithStatus, form: ActivityForm) => void;
  onNavigate: (view: ViewKey) => void;
  onOpenDeal: (dealId: string) => void;
}) {
  ...
  const [statusDetail, setStatusDetail] = useState<{ title: string; status: SalesStatus; reason: string } | null>(null);
  const [selectedDealId, setSelectedDealId] = useState<string | null>(null);
  const [selectedStage, setSelectedStage] = useState<SalesStage | null>(null);
  const [summaryPeriod, setSummaryPeriod] = useState<SalesPeriod>("month");
  const [tableViewMode, setTableViewMode] = useState<SalesTableViewMode>(loadSalesTableViewMode);
  const [showColumnSettings, setShowColumnSettings] = useState(false);
  const [visibleColumnKeys, setVisibleColumnKeys] = useState<SalesColumnKey[]>(loadSalesTableColumns);
  const [sortState, setSortState] = useState<SalesSortState>(loadSalesSortState);
  ...
  const salesGridTemplate = orderedSalesColumns.map((column) => {
    ...
    if (tableDensity === "dense") return `${width}px`;
    if (column === "customer") return `${width}px`;
    if (tableDensity === "compact" && column === "deal") return `minmax(${width}px, 1.5fr)`;
    if (column === "deal") return `${width}px`;
  ...
  function submitDeal(form: NewSalesForm) {
    onAddDeal(form);
  ...
  useEffect(() => {
    if (sortState) localStorage.setItem(salesTableSortStorageKey, JSON.stringify(sortState));
    else localStorage.removeItem(salesTableSortStorageKey);
  }, [sortState]);

  useEffect(() => {
    if (sortState && !visibleColumnKeys.includes(sortState.key)) setSortState(null);
  }, [sortState, visibleColumnKeys]);

  function toggleSalesColumn(column: SalesColumnKey) {
    if (salesColumnMeta[column].required) return;
  ...
  function handleSort(column: SalesColumnKey) {
    if (!salesColumnMeta[column].sortable) return;
  ...
  function sortIcon(column: SalesColumnKey) {
    if (!salesColumnMeta[column].sortable) return "";
    if (sortState?.key !== column) return "↕";
    return sortState.direction === "asc" ? "↑" : "↓";
  }

  function salesCellClass(column: SalesColumnKey) {
    return [
      "salesCell",
  ...
  function renderSalesCell(deal: SalesDealWithStatus, column: SalesColumnKey) {
    const primaryContact = getPrimaryContact(contacts, deal.customerId, deal.primaryContactId);
    ...
    if (column === "customer") {
      return (
        <span className="customerCell" title={noteTooltip}>
    ...
    if (column === "deal") return <span className="truncateCell" title={deal.title}>{deal.title}</span>;
    if (column === "status") {
      return (
        <span
          role="button"
          tabIndex={0}
          className={`statusTag ${statusClass(deal.computedStatus)}`}
          title={deal.computedStatusReason}
          onClick={(event) => {
            event.stopPropagation();
    ...
    if (column === "salesStage") return <span className="truncateCell" title={`${deal.salesStage} / ${deal.detailStage}`}>{deal.salesStage}</span>;
    if (column === "nextAction") return <span className="truncateCell" title={deal.nextAction || "미등록"}>{deal.nextAction || "미등록"}</span>;
    if (column === "dueDate") return <span>{deal.nextActionDueDate || "-"}</span>;
    if (column === "contact") return <span className="truncateCell" title={`${contactDetail(primaryContact)} / ${primaryContact?.phone || "연락처 미등록"} / ${primaryContact?.email || "이메일 미등록"}`}>{contactSummary(primaryContact)}</span>;
    if (column === "expectedRevenue") return <span>{formatCurrency(deal.expectedRevenue)}</span>;
    if (column === "lastContact") return <span>{deal.lastContactDate}</span>;
    if (column === "detailStage") return <span className="truncateCell" title={deal.detailStage}>{deal.detailStage}</span>;
    if (column === "phone") return <span className="truncateCell" title={primaryContact?.phone || "미등록"}>{primaryContact?.phone || "미등록"}</span>;
    if (column === "notes") {
      return (
        <span className="noteBadgeGroup" title={noteTooltip}>
          {noteLabels.length ? (
            <>
    ...
    if (column === "owner") return <span>{deal.owner}</span>;
    if (column === "industry") return <span>{deal.industry}</span>;
    if (column === "products") return <span className="truncateCell" title={deal.productModules?.join(", ") || "미등록"}>{deal.productModules?.join(", ") || "미등록"}</span>;
    if (column === "requestedDiscount") return <span>{deal.requestedDiscountRate ? `${deal.requestedDiscountRate}%` : "-"}</span>;
    if (column === "competitor") return <span>{deal.competitorInvolved ? "참여" : "없음"}</span>;
    if (column === "source") return <span className="truncateCell" title={deal.source}>{deal.source}</span>;
    ...
    <div className="featureStack">
      ...
      <section className="panel salesControlPanel">
        <div className="formGrid denseForm">
          ...
          <label>
            영업 단계
            <select value={stageFilter} onChange={(event) => setStageFilter(event.target.value as "all" | SalesStage)}>
              <option value="all">전체</option>
          ...
          <label>
            상태
            <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "all" | SalesStatus)}>
              <option value="all">전체</option>
        ...
        {showDetailedFilters && (
          <div className="detailFilterPanel">
      ...
      <section className="panel">
        ...
        <div className="pipelineGrid">
          {overview.pipeline.map((stage) => (
            <button className={`pipelineCard ${stage.riskDealCount > 0 ? "hasRisk" : ""}`} key={stage.stage} onClick={() => setSelectedStage(stage.stage)}>
      ...
      <section className="panel">
        ...
        {showColumnSettings && (
          <div className="columnSettingsPanel">
            ...
            <div className="columnOptionGrid">
              {[...defaultSalesColumnKeys, ...optionalSalesColumnKeys].map((column) => (
                <label className="checkboxLine" key={column}>
                  <input
                    type="checkbox"
                    checked={visibleColumnKeys.includes(column)}
                    disabled={Boolean(salesColumnMeta[column].required)}
                    onChange={() => toggleSalesColumn(column)}
                  />
        ...
        {tableViewMode === "card" && (
          <div className="cardSortBar">
            <label>
              카드 정렬
              <select value={cardSortOptionFromState(sortState)} onChange={(event) => setSortState(sortStateFromCardOption(event.target.value as CardSortOption))}>
        ...
        {tableViewMode === "table" ? (
          <div
            className={`salesTable ${tableDensity === "dense" ? "denseTable" : tableDensity === "compact" ? "compactTable" : "balancedTable"}`}
            role="region"
            aria-label="고객/딜 현황 테이블"
            style={{
          ...
          >
            <div className="salesHead" style={{ gridTemplateColumns: salesGridTemplate, minWidth: "var(--sales-table-min-width)" }}>
              {orderedSalesColumns.map((column) => (
                <span className={salesCellClass(column)} key={column}>
                  {salesColumnMeta[column].sortable ? (
                    <button
                      className={`sortHeaderButton ${sortState?.key === column ? "active" : ""}`}
                      type="button"
                      onClick={() => handleSort(column)}
                      title={`${salesColumnMeta[column].label} 기준 정렬`}
                    >
                      {salesColumnMeta[column].label}
            ...
            {displayedDeals.map((deal) => (
              <button
                className={`salesRow ${deal.id === selectedDealId ? "selectedRow" : ""}`}
                key={deal.id}
                style={{ gridTemplateColumns: salesGridTemplate, minWidth: "var(--sales-table-min-width)" }}
                onClick={() => {
          ...
          <div className="dealCardGrid">
            {displayedDeals.map((deal) => {
              const primaryContact = getPrimaryContact(contacts, deal.customerId, deal.primaryContactId);
              return (
                <button
                  className={`dealSummaryCard ${deal.id === selectedDealId ? "selectedRow" : ""}`}
                  key={deal.id}
                  onClick={() => {
                    setSelectedDealId(deal.id);
                    onOpenDeal(deal.id);
      ...
      <div className="splitGrid">
        ...
        <NextActionPanel
          ...
          riskDeals={overview.riskDeals}
          deals={overview.dealsWithStatus}
          onOpenDeal={(dealId) => {
            setSelectedDealId(dealId);
            onOpenDeal(dealId);
          }}
      ...
      {selectedStage && (
        <ModalShell label={`${selectedStage} 상세`} onClose={() => setSelectedStage(null)}>
          <StageDetailPanel
            stage={selectedStage}
            summary={overview.pipeline.find((item) => item.stage === selectedStage)}
            deals={overview.dealsWithStatus.filter((deal) => deal.salesStage === selectedStage)}
...
function ModalShell({ label, onClose, children }: { label: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className="modalLayer" role="dialog" aria-modal="true" aria-label={label}>
...
function DetailDrawerShell({ label, onClose, children }: { label: string; onClose: () => void; children: React.ReactNode }) {
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape") onClose();
    }
  ...
  return (
    <div className="drawerLayer" role="dialog" aria-modal="true" aria-label={label}>
...
function StageDetailPanel({
  ...
  stage: SalesStage;
  summary?: ReturnType<typeof getSalesOverview>["pipeline"][number];
  deals: SalesDealWithStatus[];
    ...
    <section className="stageDetail">
      ...
      <section className="panel inlinePanel">
        ...
        <div className="distributionGrid">
          {(summary?.detailDistribution.length ? summary.detailDistribution : detailStagesBySalesStage[stage].map((detailStage) => ({ detailStage, dealCount: 0 }))).map((item) => (
            <span key={item.detailStage}>{item.detailStage} {item.dealCount}건</span>
          ))}
        </div>
      </section>
      <section className="panel inlinePanel">
        ...
        <div className="stageDealTable">
          ...
          {deals.map((deal) => (
            <div className={`stageDealRow ${deal.id === selectedDealId ? "selectedRow" : ""}`} key={deal.id}>
...
function NewSalesDealForm({ onSubmit, onCancel }: { onSubmit: (form: NewSalesForm) => void; onCancel: () => void }) {
  const [form, setForm] = useState<NewSalesForm>(emptySalesForm);
  const [errors, setErrors] = useState<string[]>([]);

  function update<K extends keyof NewSalesForm>(key: K, value: NewSalesForm[K]) {
    setForm((current) => ({
      ...current,
      [key]: value,
      ...(key === "salesType" && value === "신규 제안" ? { salesStage: "신규 OI 발굴" as SalesStage, detailStage: "신규 제안" as SalesDetailStage } : {}),
      ...(key === "salesType" && value === "신규 인입" ? { salesStage: "신규 OI 발굴" as SalesStage, detailStage: "신규 인입" as SalesDetailStage } : {}),
      ...(key === "salesStage" ? { detailStage: detailStagesBySalesStage[value as SalesStage][0] } : {}),
    }));
  }
      ...
      <div className="formSectionStack">
        <section className="formSection">
          ...
          <div className="formGrid denseForm">
            <FieldLabel label="영업 유형" required>
              <select value={form.salesType} onChange={(event) => update("salesType", event.target.value as NewSalesForm["salesType"])}>
        ...
        <section className="formSection">
          ...
          <div className="formGrid denseForm">
            <FieldLabel label="상위 영업 단계" required>
              <select value={form.salesStage} onChange={(event) => update("salesStage", event.target.value as SalesStage)}>
                {pipelineStages.map((stage) => (
            ...
            <FieldLabel label="상세 상태" required>
              <select value={form.detailStage} onChange={(event) => update("detailStage", event.target.value as SalesDetailStage)}>
                {detailStagesBySalesStage[form.salesStage].map((stage) => (
                  <option key={stage}>{stage}</option>
                ))}
              </select>
            </FieldLabel>
        ...
        <section className="formSection">
          ...
          <div className="formGrid denseForm">
            ...
            <FieldLabel label="역할">
              <select value={form.contactRole} onChange={(event) => update("contactRole", event.target.value as CustomerContact["role"])}>
            ...
            <FieldLabel label="경쟁사 참여 여부">
              <select value={form.competitorInvolved} onChange={(event) => update("competitorInvolved", event.target.value as "no" | "yes")}>
...
function FieldLabel({
  ...
  return (
    <label className={`${required ? "requiredField" : ""} ${className ?? ""}`}>
...
function StatusRuleSettingsPanel({
  settings,
  onChange,
}: {
  settings: SalesStatusRuleSettings;
  onChange: (settings: SalesStatusRuleSettings) => void;
}) {
  const fields: Array<[keyof SalesStatusRuleSettings, string]> = [
...
function SalesFilterTabs({ value, onChange }: { value: SalesFilter; onChange: (filter: SalesFilter) => void }) {
  const filters: Array<[SalesFilter, string]> = [
    ["all", "전체"],
    ["신규 인입", "신규 인입"],
    ["신규 제안", "신규 제안"],
    ["active", "진행 중"],
  ...
  return (
    <div className="filterTabs" aria-label="영업 현황 필터">
...
function DealDetailPanel({
  ...
  deal: SalesDealWithStatus;
  activities: SalesActivity[];
  documents: SavedDocument[];
  pricingGuides: SavedPricingGuide[];
  contacts: CustomerContact[];
  customerNote: CustomerNotes;
  onClose: () => void;
  onAddActivity: (deal: SalesDealWithStatus, form: ActivityForm) => void;
  onUpdateActivity: (deal: SalesDealWithStatus, form: ActivityForm) => void;
  onDeleteActivity: (deal: SalesDealWithStatus, activityId: string) => void;
  onSaveContact: (form: CustomerContactForm) => void;
  onDeleteContact: (contactId: string) => void;
  onSetPrimaryContact: (contactId: string) => void;
  onSaveNotes: (note: CustomerNotes) => void;
  onUpdateNextAction: (deal: SalesDealWithStatus, nextAction: string, nextActionDueDate: string) => void;
  onNavigate: (view: ViewKey) => void;
}) {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "contacts" | "history" | "documents" | "pricing" | "memo">("overview");
  ...
  const recentActivities = [...activities]
    ...
    .slice(0, 8);
  const documentActivities = recentActivities.filter((activity) =>
    ["견적서 발송", "제안서 발송", "공문 발송", "추가 자료 송부"].includes(activity.activityType),
  );
      ...
      {activeTab === "history" && (
        <div className="drawerTabPanel">
          ...
          {editingActivity && (
            <ActivityFormPanel
              deal={deal}
              defaultOwner={editingActivity.owner}
              contacts={contacts}
          ...
          <div className="detailTimeline">
            {recentActivities.length === 0 ? (
              <EmptyState text="등록된 영업 이력이 없습니다." />
            ) : (
      ...
      {activeTab === "documents" && (
        <div className="drawerTabPanel">
          <section className="drawerSection">
            ...
            {documents.length > 0 && (
              <div className="documentHistoryList">
                {documents.map((document) => (
                  <article className="documentHistoryCard" key={document.id}>
            ...
            <div className="detailTimeline">
              {documentActivities.length === 0 && documents.length === 0 ? (
                <EmptyState text="등록된 문서 이력이 없습니다." />
              ) : (
                documentActivities.map((activity) => (
                  <ActivityTimelineCard
                    ...
                    onDelete={() => {
                      if (window.confirm("이 영업 이력을 삭제하시겠습니까?")) onDeleteActivity(deal, activity.id);
                    }}
                  />
                ))
              )}
      ...
      {activeTab === "pricing" && (
        <div className="drawerTabPanel">
          ...
          <section className="drawerSection">
            <h3>저장된 가격 가이드</h3>
            <div className="documentHistoryList">
              {pricingGuides.length ? pricingGuides.map((item) => (
                <article className="documentHistoryCard" key={item.id}>
              ...
              )) : <EmptyState text="저장된 가격 가이드 결과가 없습니다." />}
            </div>
          </section>
        </div>
      )}
...
function ActivityFormPanel({
  ...
  function update<K extends keyof ActivityForm>(key: K, value: ActivityForm[K]) {
    setForm((current) => ({ ...current, [key]: value }));
  }
    ...
    <div className="activityForm">
      ...
      <div className="formGrid denseForm">
        ...
        <FieldLabel label="활동 유형" required>
          <select value={form.activityType} onChange={(event) => update("activityType", event.target.value as SalesActivityType)}>
        ...
        <FieldLabel label="연락 대상">
          <select value={form.customerContactId} onChange={(event) => update("customerContactId", event.target.value)}>
            ...
            {contacts.map((contact) => (
              <option value={contact.id} key={contact.id}>{contact.name} / {contact.department || "부서 미등록"} / {contact.role}</option>
            ))}
          </select>
        </FieldLabel>
...
function ActivityTimelineCard({
  ...
  return (
    <div className="detailTimelineItem">
  ...
  return (
    <section className="panel">
      ...
      <div className="taskStack">
        {tasks.map((task) => (
          <div className={`taskItem ${selectedTasks.some((selectedTask) => selectedTask.id === task.id) ? "highlight" : ""}`} key={task.id}>
  ...
  return (
    <section className="panel">
      ...
      <div className="riskStack">
        {riskDeals.map((deal) => (
          <button className={`riskItem ${deal.customerId === selectedCustomerId ? "highlight" : ""}`} key={deal.id} onClick={() => onOpenDeal(deal.id)}>
...
function ActivityTimeline({
  ...
  activities: SalesActivity[];
  selectedActivities: SalesActivity[];
  selectedCustomerName: string;
}) {
  return (
    <section className="panel">
      ...
      <div className="timeline">
        {activities.map((activity) => (
          <div className={`timelineItem ${selectedActivities.some((item) => item.id === activity.id) ? "highlight" : ""}`} key={activity.id}>
            <time>{activity.activityDate}</time>
...
function PricingView({
  ...
  customers: Customer[];
  input: PricingInput;
  guide: ReturnType<typeof calculatePricingGuide>;
  settings: PricingGuideSettings;
  onCustomerChange: (customerId: string) => void;
  onChange: (value: Partial<PricingInput>) => void;
  onSettingsChange: (settings: PricingGuideSettings) => void;
  onSaveGuide: (record: SavedPricingGuide) => void;
}) {
  const [feedback, setFeedback] = useState<string | null>(null);
  const [showSettings, setShowSettings] = useState(false);
  ...
  return (
    <div className="featureStack">
      ...
      <section className="panel pricingResultPanel">
        ...
        {warnings.length > 0 && (
          <div className="pricingWarningBox">
            <strong>위험 경고</strong>
      ...
      <div className="pricingLayout">
        <section className="panel">
          ...
          <div className="formGrid">
            ...
            <label>
              예산 확보 여부
              <select value={input.budgetStatus} onChange={(event) => onChange({ budgetStatus: event.target.value as PricingInput["budgetStatus"] })}>
                {["확보됨", "일부 확보", "미확인", "미확보"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              구매 예정 시점
              <select value={input.purchaseTiming} onChange={(event) => onChange({ purchaseTiming: event.target.value as PricingInput["purchaseTiming"] })}>
        ...
        <section className={showAdvancedPricing ? "panel" : "panel collapsedPanel"}>
          ...
          <div className="formGrid">
            <label>
              제품군
              <select value={input.productFamily} onChange={(event) => onChange({ productFamily: event.target.value as PricingInput["productFamily"] })}>
            ...
            <label>
              고객 중요도
              <select value={input.customerImportance} onChange={(event) => onChange({ customerImportance: event.target.value as PricingInput["customerImportance"] })}>
                {["전략 고객", "일반 고객", "레퍼런스 고객", "단기 매출 고객"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              구매 가능성
              <select value={input.purchaseLikelihood} onChange={(event) => onChange({ purchaseLikelihood: event.target.value as PricingInput["purchaseLikelihood"] })}>
                {["높음", "보통", "낮음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              의사결정 단계
              <select value={input.decisionStage} onChange={(event) => onChange({ decisionStage: event.target.value as PricingInput["decisionStage"] })}>
            ...
            <label>
              경쟁사 가격 수준
              <select value={input.competitorPriceLevel} onChange={(event) => onChange({ competitorPriceLevel: event.target.value as PricingInput["competitorPriceLevel"] })}>
                {["당사보다 낮음", "유사", "당사보다 높음", "미확인"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              경쟁 강도
              <select value={input.competitionIntensity} onChange={(event) => onChange({ competitionIntensity: event.target.value as PricingInput["competitionIntensity"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              기존 사용 솔루션
              <select value={input.existingSolution} onChange={(event) => onChange({ existingSolution: event.target.value as PricingInput["existingSolution"] })}>
            ...
            <label>
              구축 난이도
              <select value={input.implementationDifficulty} onChange={(event) => onChange({ implementationDifficulty: event.target.value as PricingInput["implementationDifficulty"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              기술 검토 리스크
              <select value={input.technicalRisk} onChange={(event) => onChange({ technicalRisk: event.target.value as PricingInput["technicalRisk"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              결제 조건
              <select value={input.paymentTerms} onChange={(event) => onChange({ paymentTerms: event.target.value as PricingInput["paymentTerms"] })}>
            ...
            <label>
              레퍼런스 확보 가치
              <select value={input.referenceValue} onChange={(event) => onChange({ referenceValue: event.target.value as PricingInput["referenceValue"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              향후 확장 가능성
              <select value={input.expansionPotential} onChange={(event) => onChange({ expansionPotential: event.target.value as PricingInput["expansionPotential"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              장기 고객 가능성
              <select value={input.longTermPotential} onChange={(event) => onChange({ longTermPotential: event.target.value as PricingInput["longTermPotential"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
              매출 목표 기여도
              <select value={input.revenueTargetContribution} onChange={(event) => onChange({ revenueTargetContribution: event.target.value as PricingInput["revenueTargetContribution"] })}>
                {["낮음", "보통", "높음"].map((option) => <option key={option}>{option}</option>)}
              </select>
            </label>
            <label>
      ...
      <section className="panel negotiationPanel">
        ...
        <div className="defenseGrid">
          {[...guide.defenseLogic, ...guide.alternativeTerms].map((item) => (
            <div className="defenseItem" key={item}>
      ...
      {showSettings && (
        <ModalShell label="가격 기준 설정" onClose={() => setShowSettings(false)}>
...
function PricingReferenceList({ references, input }: { references: ReturnType<typeof calculatePricingGuide>["similarCases"]; input: PricingInput }) {
  const [filter, setFilter] = useState<"all" | "win" | "loss" | "industry" | "scale">("all");
  const filteredReferences = references.filter((reference) => {
    if (filter === "win") return reference.result === "win";
    if (filter === "loss") return reference.result === "loss";
    if (filter === "industry") return reference.industry === input.industry;
    if (filter === "scale") return Math.abs(reference.userCount - input.userCount) <= Math.max(500, input.userCount * 0.35);
    return true;
  });
  return (
    <section className="panel">
      <div className="panelHeader">
        ...
        <div className="filterTabs">
          {[
            ...
            <button className={filter === key ? "active" : ""} key={key} onClick={() => setFilter(key as typeof filter)}>{label}</button>
          ))}
        </div>
      </div>
...
function DiscountVisualization({ input, guide }: { input: PricingInput; guide: ReturnType<typeof calculatePricingGuide> }) {
...
function getPriceStatus(input: PricingInput, guide: ReturnType<typeof calculatePricingGuide>, settings: PricingGuideSettings) {
  if (input.requestedDiscountRate > guide.maxAllowedDiscountRate || guide.expectedMarginRate < settings.marginFloorRate) return "위험";
  if (input.requestedDiscountRate > guide.recommendedDiscountRate + 2 || guide.marginRiskLevel === "주의") return "주의";
  return "정상";
}

function buildPricingSummary(input: PricingInput, guide: ReturnType<typeof calculatePricingGuide>) {
...
function getPricingWarnings(input: PricingInput, guide: ReturnType<typeof calculatePricingGuide>, settings: PricingGuideSettings) {
  return [
...
function PricingSettingsPanel({
  ...
  settings: PricingGuideSettings;
  onSave: (settings: PricingGuideSettings) => void;
  onReset: () => void;
}) {
  const [draft, setDraft] = useState<PricingGuideSettings>(settings);
  const [activeTab, setActiveTab] = useState<"discount" | "margin" | "probability" | "reference">("discount");
  const [error, setError] = useState("");

  function update<K extends keyof PricingGuideSettings>(key: K, value: PricingGuideSettings[K]) {
    setDraft((current) => ({ ...current, [key]: value }));
  }

  function updateNested<T extends "industryDiscountWeight" | "winProbability" | "referenceWeights">(
    group: T,
    key: keyof PricingGuideSettings[T],
    value: number,
  ) {
    setDraft((current) => ({
      ...current,
  ...
  return (
    <section className="settingsPanel">
      ...
      {activeTab === "probability" && (
        <div className="formGrid">
          {Object.entries(draft.winProbability).map(([key, value]) => (
            <NumberSetting key={key} label={probabilitySettingLabels[key] ?? key} value={value} onChange={(nextValue) => updateNested("winProbability", key as keyof PricingGuideSettings["winProbability"], nextValue)} />
          ))}
        </div>
      )}
      ...
      {activeTab === "reference" && (
        <div className="formGrid">
          {Object.entries(draft.referenceWeights).map(([key, value]) => (
            <NumberSetting key={key} label={referenceSettingLabels[key] ?? key} value={value} onChange={(nextValue) => updateNested("referenceWeights", key as keyof PricingGuideSettings["referenceWeights"], nextValue)} />
          ))}
        </div>
      )}
...
const probabilitySettingLabels: Record<string, string> = {
  baseRate: "기본 수주 확률",
...
const referenceSettingLabels: Record<string, string> = {
...
function CaseList({ title, cases, table = false }: { title: string; cases: WinLossCase[]; table?: boolean }) {
  return (
    <section className={table ? "panel" : "caseSection"}>
      <div className="panelTitle">
        <BriefcaseBusiness size={18} />
        <h2>{title}</h2>
      </div>
...
function DraftPreview({ draft }: { draft: DocumentDraft }) {
  if (draft.documentType === "공문") return <OfficialLetterPreview draft={draft} />;
  if (draft.documentType === "메일") return <EmailDraftPreview draft={draft} />;
  if (draft.documentType === "견적서") return <QuoteDraftPreview draft={draft} />;
  if (draft.documentType === "제안서") return <ProposalDraftPreview draft={draft} />;
  return (
    <article className="draftPaper">
      <h3>{draft.title}</h3>
      ...
      {draft.sections.map((section) => (
        <DraftSection key={section.label} label={section.label} value={section.value} />
      ))}
    </article>
  );
}

function EmailDraftPreview({ draft }: { draft: DocumentDraft }) {
  const pick = (label: string) => draft.sections.find((section) => section.label === label)?.value || "";
  return (
    <article className="draftPaper emailPreview">
      <div className="draftMeta">
        <span>{draft.documentType}</span>
        <span>{draft.templateName}</span>
        <span>{draft.audience}</span>
...
function QuoteDraftPreview({ draft }: { draft: DocumentDraft }) {
  const pick = (label: string) => draft.sections.find((section) => section.label === label)?.value || "";
  return (
    <article className="draftPaper quotePreview">
      <div className="draftMeta">
        <span>{draft.documentType}</span>
        <span>{draft.templateName}</span>
        <span>{draft.audience}</span>
...
function ProposalDraftPreview({ draft }: { draft: DocumentDraft }) {
  return (
    <article className="draftPaper proposalPreview">
      <div className="draftMeta">
        <span>{draft.documentType}</span>
        <span>{draft.templateName}</span>
        <span>{draft.audience}</span>
...
function OfficialLetterPreview({ draft }: { draft: DocumentDraft }) {
  const sectionValue = (labels: string[]) => draft.sections.find((section) => labels.includes(section.label))?.value || "";
  const officialBodyText = sectionValue(["본문", "蹂몃Ц"]) || draft.body;
  const officialBodyLines = officialBodyText.split("\n");
  const officialMetaLines = officialBodyLines.filter((line) =>
    ["문서번호", "시행일자", "수    신", "참    조", "제    목"].some((label) => line.startsWith(label)),
  );
  const officialFooterLine = officialBodyLines.find((line) => line.includes("대표이사")) || "CRM 주식회사 대표이사";
  const officialContentLines = officialBodyLines.filter((line) =>
  ...
  return (
    <article className="officialLetterPaper">
      ...
      <div className="officialMeta">
        {officialMetaLines.map((line) => (
          <p key={line}>{line}</p>
        ))}
      </div>
      <div className="officialBody">
        {officialContentLines.map((line) => {
          if (line.includes("아 래")) return <strong className="officialCenter" key={line}>{line}</strong>;
          if (/^[가-라]\./.test(line.trim())) return <p className="officialBelowItem" key={line}>{line}</p>;
...
function matchesSalesFilter(deal: SalesDealWithStatus, filter: SalesFilter) {
  if (filter === "all") return true;
  if (filter === "신규 인입" || filter === "신규 제안") return deal.salesType === filter || deal.detailStage === filter;
  if (filter === "active") return !["수주", "실패", "보류"].includes(deal.computedStatus);
  return deal.computedStatus === filter;
}

function typeClass(type: SalesDeal["salesType"]) {
  return {
    "신규 인입": "inbound",
    "신규 제안": "new_proposal",
...
function existingCustomerStatusClass(status: ExistingCustomer["status"]) {
  return {
    정상: "normal",
    "갱신 예정": "caution",
    "만료 임박": "delayed",
...
function createExistingCustomerFromWonDeal(deal: SalesDealWithStatus, activity: SalesActivity): ExistingCustomer {
...
function activityTypeClass(type: SalesActivityType) {
  if (["수주", "계약 완료", "발주 완료"].includes(type)) return "win";
  if (["실패", "보류"].includes(type)) return "closed";
  if (["견적서 발송", "제안서 발송", "공문 발송"].includes(type)) return "document";
  if (["전화", "이메일", "미팅"].includes(type)) return "contact";
  if (["기술 검토", "보안 검토", "내부 검토", "계약 협의"].includes(type)) return "review";
  return "default";
}

function getMissingSalesFields(form: NewSalesForm) {
  const checks: Array<[keyof NewSalesForm, string]> = [
    ["salesType", "영업 유형"],
    ["customerName", "고객사명"],
    ["title", "영업 건명"],
    ["industry", "산업군"],
...
function getMissingActivityFields(form: ActivityForm) {
  const checks: Array<[keyof ActivityForm, string]> = [
    ["activityDate", "활동일"],
    ["activityType", "활동 유형"],
    ["title", "제목"],
    ["owner", "담당자"],
...
function valueOrMissing(value?: string | number | null) {
  if (value === undefined || value === null) return "미등록";
  const text = String(value).trim();
  return text.length ? text : "미등록";
}

function addTaskFromDeal(tasks: SalesTask[], deal: SalesDeal) {
  if (!deal.nextAction || !deal.nextActionDueDate) return tasks;
  return [
    {
      id: `task-${deal.id}`,
      dealId: deal.id,
