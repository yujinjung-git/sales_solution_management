...
export function getSupportedExportFormats(documentType) {
    return documentType === "견적서" ? ["PDF", "Word", "Excel"] : ["PDF", "Word"];
}
export function recommendedExportFormat(documentType) {
    if (documentType === "견적서")
        return "Excel";
    if (documentType === "제안서")
        return "PDF";
...
export function buildExportFileName(customer, deal, documentType, format, date = new Date(), templateName) {
...
export async function exportDocument(draft, format, customer, deal) {
    ...
    if (format === "Excel")
        exportToExcel(draft, fileName);
    if (format === "PDF")
        exportToPdf(draft, fileName);
    if (format === "Word")
        await exportToWord(draft, fileName);
    return fileName;
}
export function exportToPdf(draft, fileName) {
...
export async function exportToWord(draft, fileName) {
    const isOfficialLetter = draft.body.includes("문서번호") || draft.body.includes("수    신");
...
export function exportToExcel(draft, fileName) {
...
function quoteRowsFromDraft(draft) {
...
export function createQuoteTableRows(draft) {
...
function splitText(text, max = 80) {
    return text.split("\n").flatMap((line) => {
        if (line.length <= max)
            return [line];
        const chunks = [];
