...
export const defaultSalesStatusSettings: SalesStatusRuleSettings = {
...
export const pipelineStages: SalesStage[] = [
  "신규 OI 발굴",
  "요구사항 확인",
  "제안/견적",
  "검토/협상",
...
export const detailStagesBySalesStage: Record<SalesStage, SalesDetailStage[]> = {
...
const closedStatuses = new Set<SalesStatus>(["수주", "실패"]);
...
export type PipelineStageSummary = {
  ...
  detailDistribution: Array<{ detailStage: SalesDetailStage; dealCount: number }>;
  representativeCustomers: string[];
};

export type SalesDealWithStatus = SalesDeal & {
  computedStatus: SalesStatus;
  computedStatusReason: string;
};

export type RiskDeal = SalesDealWithStatus & {
  riskReasons: string[];
};
...
export type SalesOverview = {
  ...
  dealsWithStatus: SalesDealWithStatus[];
};

export type SalesPeriod = "month" | "year";
...
export function getSalesOverview(
  ...
  settings: SalesStatusRuleSettings = defaultSalesStatusSettings,
): SalesOverview {
...
export function getSalesPeriodSummary(
...
export function getDealStatusReason(
  deal: SalesDeal,
  activities: SalesActivity[],
  tasks: SalesTask[],
  settings: SalesStatusRuleSettings,
...
export function deriveDealFromActivity(deal: SalesDeal, activity: SalesActivity): SalesDeal {
  ...
  const terminalStatus =
    activity.activityType === "수주" || activity.activityType === "계약 완료" || activity.activityType === "발주 완료"
      ? "수주"
...
export function getPipelineSummary(deals: SalesDeal[]): PipelineStageSummary[] {
  return pipelineStages.map((stage) => {
    ...
    return {
      ...
      riskDealCount: stageDeals.filter((deal) => {
        const status = "computedStatus" in deal ? (deal as SalesDealWithStatus).computedStatus : deal.status;
...
export function getRiskDeals(deals: SalesDealWithStatus[]): RiskDeal[] {
  return deals
...
export function getUpcomingTasks(tasks: SalesTask[], baseDate: Date = today): SalesTask[] {
  return tasks
...
export function getRecentActivities(activities: SalesActivity[]): SalesActivity[] {
  return [...activities]
...
export function getDealActivities(dealId: string, activities: SalesActivity[]): SalesActivity[] {
  return activities
...
function evaluateStatus(
  ...
  settings: SalesStatusRuleSettings,
  baseDate: Date,
): { status: SalesStatus; reason: string } {
...
function stageFromActivity(
  ...
  currentDetailStage: SalesDetailStage,
): { salesStage: SalesStage; detailStage: SalesDetailStage } {
  if (activityType === "신규 인입") return { salesStage: "신규 OI 발굴", detailStage: "신규 인입" };
  if (activityType === "신규 제안") return { salesStage: "신규 OI 발굴", detailStage: "신규 제안" };
  if (activityType === "기술 검토") return { salesStage: "검토/협상", detailStage: "기술 검토" };
  if (activityType === "보안 검토") return { salesStage: "검토/협상", detailStage: "보안 검토" };
  if (activityType === "견적서 발송") return { salesStage: "제안/견적", detailStage: "견적 발송" };
  if (activityType === "제안서 발송") return { salesStage: "제안/견적", detailStage: "제안 완료" };
  if (activityType === "추가 자료 송부") return { salesStage: "제안/견적", detailStage: "추가 자료 송부" };
  if (activityType === "공문 발송") return { salesStage: "제안/견적", detailStage: "공문 송부" };
  if (activityType === "계약 협의") return { salesStage: "계약 진행", detailStage: "계약 조건 확인" };
  if (activityType === "계약 완료") return { salesStage: "계약 완료(수주)", detailStage: "계약 완료" };
  if (activityType === "발주 완료") return { salesStage: "계약 완료(수주)", detailStage: "발주 완료" };
  if (activityType === "수주") return { salesStage: "계약 완료(수주)", detailStage: "수주" };
  if (activityType === "실패") return { salesStage: "종료", detailStage: "실패" };
  if (activityType === "보류") return { salesStage: "종료", detailStage: "보류" };
  return { salesStage: currentStage, detailStage: currentDetailStage };
}

function isDealInPeriod(deal: SalesDeal, period: SalesPeriod, baseDate: Date) {
  ...
  return (
    (firstContact.getFullYear() === year && firstContact.getMonth() === month) ||
    (expectedClose?.getFullYear() === year && expectedClose.getMonth() === month)
  );
}

function isWonDeal(deal: SalesDealWithStatus) {
  return deal.computedStatus === "수주" || deal.salesStage === "계약 완료(수주)";
}

function isFailedOrHoldDeal(deal: SalesDealWithStatus) {
  return deal.computedStatus === "실패" || deal.computedStatus === "보류" || deal.salesStage === "종료";
}

function isActiveDeal(deal: SalesDealWithStatus) {
  return !isWonDeal(deal) && !isFailedOrHoldDeal(deal);
}

function isExcludedFromExpectedRevenue(deal: SalesDealWithStatus) {
  return isFailedOrHoldDeal(deal) || ["장기 미응답", "예산 취소", "경쟁사 수주", "고객사 내부 중단"].includes(deal.detailStage);
}

function statusReasonForTerminal(status: SalesStatus) {
  if (status === "수주") return "계약 또는 발주가 완료된 상태입니다.";
  if (status === "실패") return "영업 기회가 실패 처리되었습니다.";
  if (status === "보류") return "고객 또는 내부 사유로 진행이 일시 중단된 상태입니다.";
  return "";
}

function getLatestContactDate(deal: SalesDeal, activities: SalesActivity[]) {
  const latestContact = latestActivityDate(
    activities.filter((activity) => contactActivityTypes.has(activity.activityType)),
  );
  return latestContact ?? deal.lastContactDate;
}

function latestActivityDate(activities: SalesActivity[], types?: SalesActivityType[]) {
...
function daysSince(dateText: string, baseDate: Date) {
  return Math.max(0, Math.floor((baseDate.getTime() - new Date(`${dateText}T00:00:00+09:00`).getTime()) / 86400000));
}

function daysBetween(baseDate: Date, dateText: string) {
