...
export function buildDocumentContext(customer, deal, license, salesDeal, activities, pricingGuide, cases) {
    return { customer, deal, license, salesDeal, activities, pricingGuide, cases };
}
export function generateDocumentDraft(input, context) {
    if (input.documentType === "메일")
        return generateEmailDraft(input, context);
    if (input.documentType === "견적서")
        return generateQuoteDraft(input, context);
    if (input.documentType === "공문")
        return generateOfficialLetterDraft(input, context);
    return generateProposalDraft(input, context);
}
export function generateEmailDraft(input, context) {
    const customerName = displayCustomer(context.customer);
    const activities = selectedActivityLines(input, context);
    const include = (name) => shouldInclude(input, name);
...
export function generateQuoteDraft(input, context) {
    const customerName = displayCustomer(context.customer);
    ...
    return makeDraft(input, `${customerName} ${context.deal.name} 견적서 초안`, [
        ...
        ...(input.audience === "내부용" && pricing ? [["내부 가격 메모", pricing.internalApprovalMessage]] : []),
    ], body);
}
export function generateOfficialLetterDraft(input, context) {
    return generateKicaOfficialLetterDraft(input, context);
}
function generateKicaOfficialLetterDraft(input, context) {
    const customerName = displayCustomer(context.customer);
...
function generateOfficialLetterDraftLegacy(input, context) {
    const customerName = displayCustomer(context.customer);
...
function generateProposalDraft(input, context) {
    const pricing = input.includePricingGuide && context.pricingGuide ? context.pricingGuide : null;
...
function makeDraft(input, title, sectionPairs, body) {
    return {
        id: `doc-${Date.now()}`,
...
function selectedActivityLines(input, context) {
...
function shouldInclude(input, name) {
    if (!input.includeItems?.length)
        return true;
    return input.includeItems.includes(name);
}
function toneLead(tone, context) {
    if (tone === "간결함")
        return "문의주신 구성은 적용 가능 여부를 아래 조건 중심으로 확인하면 됩니다.";
    if (tone === "영업 친화적")
        return "현재 검토 중인 구성은 향후 인증 범위 확장까지 고려할 때 적합한 방향입니다.";
    if (tone === "기술 설명 중심")
        return `${context.customer.environment.join(", ")} 조건을 기준으로 기술 검토 의견을 정리했습니다.`;
    if (tone === "내부 보고용")
        return "본 건은 기술 검토와 가격 협상 전 고객 예산 확인이 필요한 상태입니다.";
